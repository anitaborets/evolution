...
public class LiveDay {
    ...
    public static ArrayList<Alive> liveDay(ArrayList<Alive>[][] gameField) throws FileNotFoundException {
        StatisticReport dayReport = new StatisticReport();
        report.setLength(0);

        report.append(listOfAnimals.size()).append(" entities started. They are:").append("\n")
    ...
    private static void clearDayListOfAnimals() {
        for (int row = 1; row < gameField.length - 1; row++) {
            for (int column = 1; column < gameField[0].length - 1; column++) {
    ...
    private static void eat() throws FileNotFoundException {
        for (int row = 1; row < gameField.length - 1; row++) {
            for (int column = 1; column < gameField[0].length - 1; column++) {
    ...
    private static void moveToAnotherLocation() {
